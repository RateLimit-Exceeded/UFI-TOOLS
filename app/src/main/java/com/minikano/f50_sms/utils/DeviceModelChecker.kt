...
object DeviceModelChecker {
    ...
    private val firmwareWhiteList = listOf(
        "MU5352_DSV1.0.0B07",
        "MU5352_DSV1.0.0B05",
        "MU5352_DSV1.0.0B03",
        "MU300",
    ...
    suspend fun checkBlackList(context: Context): Boolean {
        Log.d("kano_ZTE_LOG_devcheck", "Checking local blacklist/whitelist...")

        val model = Build.MODEL.trim()
        val firmwareVersion = Build.DISPLAY

        isUnSupportDevice = devicesBlackList.any { it.trim().contains(model) }

        if (firmwareWhiteList.any { firmwareVersion.contains(it.trim()) }) {
    ...
    fun checkIsNotUFI(context: Context): Boolean {
        ...
        Log.d(
            "kano_ZTE_LOG_devcheck",
            "isUFI_0=$isUFI_0,has com.zte.web? :${isUFI?.contains("com.zte.web")} ",
        )
